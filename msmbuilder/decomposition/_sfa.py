...
class SFA(TransformerMixin, BaseEstimator):
    ...
    def _fit_standard_method(self, X):
        ...
        ## return eigenvectors and eigenvalue
        #self.eigenvectors_whiten = self.pca_whiten_.components_
        #self.eigenvalue_whiten = self.pca_whiten_.explained_variance_

        #self.eigenvectors_whiten_pca_diff = self.pca_diff_.components_
        #self.eigenvalue_whiten_pca_diff_ = self.pca_diff_.explained_variance_
    ...
    def summarize(self):
        return '\n'.join([
            ...
            "Delta Values : {delta_values_}",
            "Non trivial components: {n_nontrivial_components_}"
        ]).format(**self.__dict__)

    # experimental, don't use
    def return_eigens(self):
        return '\n'.join([
            "Slow Feature Analysis (SFA) eigenvalue and eigenvector",
            "----------",
